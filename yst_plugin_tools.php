...
if (!class_exists('Yoast_Plugin_Admin')) {
	class Yoast_Plugin_Admin {
		...
		function news() {
			...
			if ( $rss = fetch_rss( 'http://feeds.feedburner.com/elektroelch' ) ) {
				$content = '<ul>';
				$rss->items = array_slice( $rss->items, 0, 5 );
				foreach ( (array) $rss->items as $item ) {
					$content .= '<li class="yoast">';
					$content .= '<a class="rsswidget" href="'.clean_url( $item['link'], $protocolls=null, 'display' ).'">'. wp_specialchars($item['title']) .'</a> ';
					$content .= '</li>';
				}
				$content .= '<li class="rss"><a href="http://elektroelch.de/blog/feed/">Subscribe with RSS</a></li>';
				// $content .= '<li class="email"><a href="http://yoast.com/email-blog-updates/">Subscribe by email</a></li>';
				Yoast_Plugin_Admin::postbox('yoastlatest', 'Latest news from elektroelch', $content);
			} else {
				Yoast_Plugin_Admin::postbox('elektroelchlatest', 'Latest news from elektroelch', 'Nothing to say...');
			}
		}
